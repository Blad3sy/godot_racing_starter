...
func input():
	var steering = Input.get_action_strength("steer_right"+PLAYER_TYPE) - Input.get_action_strength("steer_left"+PLAYER_TYPE)
	if Input.is_action_pressed("drift"+PLAYER_TYPE):
		apply_torque_impulse(DRIFT_STEERING * steering)
		linear_damp = DRIFT_FRICTION
		$skid.stream_paused = false
		doSkidmark()
	else:
		var acceleration = (Input.get_action_strength("accelerate"+PLAYER_TYPE) - Input.get_action_strength("brake"+PLAYER_TYPE)) * Vector2.UP * ACCELERATION
...
func camera():
	var scalefactor = 2 + linear_velocity.length()/1000
